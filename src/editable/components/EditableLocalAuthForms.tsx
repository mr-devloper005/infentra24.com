...
const inputClass = 'h-12 rounded-xl border border-white/12 bg-white/[0.04] px-4 text-base text-white outline-none transition placeholder:text-white/40 focus:border-[var(--slot4-accent)]'
const buttonClass = 'inline-flex h-12 items-center justify-center rounded-full bg-[var(--slot4-accent-gradient)] px-6 text-sm font-semibold text-white shadow-[0_12px_40px_-12px_rgba(87,73,100,0.8)] transition hover:-translate-y-0.5 disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-xl px-4 py-3 text-sm font-medium ${status === 'success' ? 'border border-emerald-400/30 bg-emerald-400/10 text-emerald-200' : 'border border-red-400/30 bg-red-400/10 text-red-200'}`}>{message}</p> : null}
